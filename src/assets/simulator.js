...
export class GameState {
  ...
  build(nodes, edges) {
    ...
    for (const choice of choices) {
      if (choice.gain[me] > maxGain) {
        chosen.length = 0;
        chosen.push(choice);
        maxGain = choice.gain[me];
      } else if (choice.gain[me] === maxGain) {
        chosen.push(choice);
      }
    }
    this.gain = new Array(this.option.players);
    ...
    for (let i = 0; i < this.option.players; i++) {
      for (const choice of chosen) {
        this.gain[i] += choice.gain[i];
      }
      this.gain[i] /= chosen.length;
    }

    for (const choice of chosen) {
      choice.edge.arrows = {to: true};
    }
